...
<%@page import="greenInvoice.maps.Glassware"%>
...
<%
    ...
    ArrayList<Glassware> products = dm.getGlass();
...
<html>
    <head>
        ...
        <script>
            ...
            function doCall(a)
            {
                $.ajax({
                    type: "GET",
                    url: "GetProduct?tp=glassware&productid=" + a,
                    async: true,
    ...
    <body class="easyui-layout" style="font-family: Calibri;">
        <div data-options="region: 'north', border: false" style="padding: 10px; height: 48%;">
            <div class="easyui-panel" style="padding: 10px;width: 100%; overflow: hidden; height: 50px;background-repeat: no-repeat; background-image: url('images/glass.png');background-size: 3%; background-position: 1% 50%;" >
                <h2 style="margin-left: 50px;margin-top: 2px;">Glassware</h2>
            </div><br/>
            <form action="AddVoucher" method="post">
                <input type="hidden" name="email" value="<%= email%>"/>
                <input type="hidden" name="producttp" value="glassware"/>
                <input type="hidden" name="pg" value="\WEB-INF\sellGlass.jsp?email=<%=email%>"/>
                <div class="easyui-panel" title="Invoice Details">
                    <table style="height: 100%;width: 100%;border: 0pt;" cellpadding="3" cellspacing="3">
                        <tr>
                            ...
                            <td align="center" width="10%"><input class="easyui-numberbox" id="invno" name="invno"/></td> <!-- getDet(this.value); onchange="$('#getInvNo').value = this.value;"/></td-->
                            <td align="center" width="25%">Buyer:</td>
                            <td align="center" width="25%" style="border-right: 1pt solid;">
                                <select name="partyno" id="partyno" class="easyui-combobox" style="width: 200px;" editable="false">
                                    <%                                while (p.hasNext()) {
                        ...
                        <tr>
                            ...
                            <td align="center" width="30%" style="border-right: 1pt solid;">
                                <select name="productid" style="width: 310px;" onChange="doCall(this.value);">
                                    ...
                                    <%
                                        while (i.hasNext()) {
                                            Glassware chem = (Glassware) i.next();

                                    %>
                                    <option value="<%= chem.getProductId()%>"><%= chem.getDescGoods() + " , " + chem.getCatNo() + ", " + chem.getMake()%></option>
                                    <%  }
                                    %>
                                </select>
                            </td>
