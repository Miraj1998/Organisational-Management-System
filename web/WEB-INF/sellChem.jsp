...
<html>
    <head>
        ...
        <script>
            ...
            function doCall(a)
            {
                $.ajax({
                    type: "GET",
                    url: "GetProduct?tp=chemicals&productid=" + a,
                    async: true,
                    success: function (data)
                    {
                        //alert (data);
    ...
    <body class="easyui-layout" style="font-family: Calibri;">
        <div data-options="region: 'north', border: false" style="padding: 10px; height: 48%;">
            <div class="easyui-panel" style="padding: 10px;width: 100%; height: 50px;background-repeat: no-repeat; overflow: hidden; background-image: url('images/chemicals.png');background-size: 3%; background-position: 1% 50%;" >
                <h2 style="margin-left: 50px;margin-top: 2px;">Chemicals</h2>
            </div><br/>
            <form action="AddVoucher?tp=chemicals" method="post">
                <input type="hidden" name="email" value="<%= email%>"/>
                <input type="hidden" name="producttp" value="chemicals"/>
                <input type="hidden" name="pg" value="\WEB-INF\sellChem.jsp?email=<%=email%>"/>
                        ...
                        <tr>
                            ...
                            <td align="center" width="30%" style="border-right: 1pt solid;">
                                <select name="productid" style="width: 310px;" onChange="doCall(this.value);">
                                    ...
                                    %>
                                    <option value="<%= chem.getProductcode()%>"><%= chem.getProductdesc() + " , " + chem.getSpecification() + ", " + chem.getPacksize()%></option>
                                    <%  }
                                    %>
                                </select>
                            </td>
